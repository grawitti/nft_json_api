...
int main(int argc, char const **argv)
{
    ...
    default:
        printf("Need args: family table chain ifname protocol saddr sa_len sport_begin sport end daddr_addr da_len dport_begin dport_end policy\n");
        return -1;
    }
    ...
    json_t *nft_expr = nft_json_build_expr_policy(&pol_ctx, r_ctx.ch_ctx.family, 0, &err);
    if (!nft_expr)
        return -1;
